...
-- The issue is that even with SECURITY DEFINER, the function might not be
-- bypassing RLS on the organizations table properly.

-- Drop and recreate with explicit RLS bypass
DROP FUNCTION IF EXISTS public.user_can_manage_seating_for_location(uuid);
