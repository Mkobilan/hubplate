                                                ...
                                                {openMenuId === card.id && (
                                                    <div className="absolute right-0 mt-2 w-48 rounded-xl bg-slate-900 border border-slate-800 shadow-xl z-50 py-1 animate-in fade-in zoom-in-95 duration-100 origin-top-right">
                                                        ...
                                                        <button
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                                handleDeactivate(card);
                                                                setOpenMenuId(null);
                                                            }}
                                                            disabled={!card.is_active || deactivatingId === card.id}
                                                            className={cn(
                                                                "flex items-center w-full px-4 py-2.5 text-sm transition-colors",
                                                                card.is_active ? "text-red-500 hover:bg-red-500/10" : "text-slate-600 cursor-not-allowed opacity-50"
                                                            )}
                                                        >
                                                            {deactivatingId === card.id ? (
                                                                <Loader2 className="h-4 w-4 mr-3 animate-spin" />
                                                            ) : (
                                                                <Trash2 className="h-4 w-4 mr-3" />
                                                            )}
                                                            {card.is_active ? "Deactivate Card" : "Card Inactive"}
                                                        </button>
                                                    </div>
                                                )}
                                            </div>
